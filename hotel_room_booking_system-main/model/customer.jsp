...
<%
	...
	if((request.getParameter("act")).equals("chk_login"))
	{
		if(customer.checkLogin(request.getParameter("login_user"),request.getParameter("login_password")))
		{
			...
			session.setAttribute("customer_mobile",Values.get("customer_mobile"));
			session.setAttribute("customer_email",Values.get("customer_email"));
			...
			if(session.getAttribute("room_id") == null) {
				...
				response.sendRedirect("../RoomDetails.jsp?room_id="+session.getAttribute("room_id"));
			}
		}
		else
		{
